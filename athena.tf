...
resource "aws_glue_classifier" "athena_alb_partitioned_classifier" {
  ...
  grok_classifier {
    ...
    grok_pattern = "%%{WORD:type} %%{TIMESTAMP_ISO8601:timestamp} %%{DATA:alb_name} %%{IP:client_ip}:%%{NUMBER:client_port:int} %%{IP:target_ip}:%%{NUMBER:target_port:int} %%{NUMBER:request_processing_time:float} %%{NUMBER:target_processing_time:float} %%{NUMBER:response_processing_time:float} %%{NUMBER:elb_status_code:int} %%{NUMBER:target_status_code:int} %%{NUMBER:received_bytes:int} %%{NUMBER:sent_bytes:int} \\\"%%{WORD:http_method} %%{URI:request_url} HTTP/%%{NUMBER:http_version}\\\" \\\"%%{DATA:user_agent}\\\" %%{DATA:ssl_cipher} %%{DATA:ssl_protocol} %%{DATA:target_group_arn} \\\"%%{DATA:trace_id}\\\" %%{DATA:domain_name} %%{DATA:chosen_cert_arn} %%{NUMBER:matched_rule_priority:int} %%{TIMESTAMP_ISO8601:request_creation_time} \\\"%%{WORD:actions_executed}\\\" %%{DATA:redirect_url} %%{DATA:error_reason} \\\"%%{IP:target_ip_2}:%%{NUMBER:target_port_2:int}\\\" \\\"%%{NUMBER:target_status_code_2:int}\\\" %%{DATA:classification} %%{DATA:classification_reason} %%{GREEDYDATA:conn_trace_id}"
  }
}
